...
A release is cut by bumping either `VERSION` (the upstream FFmpeg version) or `BUILD-NUMBER` (when only the build flags change) on `main`. The GitHub Actions workflow at `.github/workflows/build.yml` runs the platform-specific build script for every supported target, uploads the resulting binaries as a tagged release, and writes a `RELEASE-NOTES.md` recording the SHA256 of each artifact.

Tag format: `v<ffmpeg-version>-<build-number>`, e.g. `v8.1.1-1`.
...
3. Edit `VERSION` to the new value, e.g. `8.2.0`. Reset `BUILD-NUMBER` to `1`.
4. Edit `scripts/build-macos.sh` if the expected tarball SHA256 changed (it's pinned in the script as a defence-in-depth check).
5. Open a PR. CI runs the workflow on a draft tag for verification.
6. Merge to `main`. The push triggers the release workflow, producing `v<new-version>-1`.

## How to bump only the build flags

Bump `BUILD-NUMBER` (no `VERSION` change). The new tag will be `v<existing-version>-<new-build-number>` and the release notes call out the flag delta.
